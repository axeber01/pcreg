...
classdef myLidarMapBuilder < handle
    ...
    methods
        %------------------------------------------------------------------
        function this = myLidarMapBuilder(varargin)
        ...
        function optimizeMapPoses(this, varargin)
            ...
            %
            %   Note: This method requires that you have access to the
            %   Navigation Toolbox(TM).
            %
            %   See also createPoseGraph, optimizePoseGraph, optimizePoses.
            
            if this.Verbose
                fprintf('Optimizing pose graph...');
            end
            
            % Create pose graph from view set
            G = createPoseGraph(this.ViewSet);
            
            % Optimize pose graph using g2o
            Goptim = optimizePoseGraph(G, 'g2o-levenberg-marquardt', varargin{:});
            
            % Update view set with optimized poses
            this.ViewSet = updateView(this.ViewSet, Goptim.Nodes);
        ...
        function rebuildMap(this, radius, NameValues)
            ...
            for n = 1 : skipFrames: numFrames
                ptCloud = ptClouds(n);
                ptCloud = pcdownsample(this.processPointCloud(ptCloud), ...
                    'random', this.DownsamplePercent);
                ptCloud = select(ptCloud, findNeighborsInRadius(ptCloud, [0 0 0], radius));
                ptClouds(n) = pctransform(ptCloud, absTforms(n));
            end
            
            %ptClouds = arrayfun(@this.processPointCloud, ptClouds);
            %ptClouds = arrayfun(@(in)pcdownsample(in,'random',this.DownsamplePercent), ptClouds);
            %ptClouds = arrayfun(@(in)select(in,findNeighborsInRadius(in, [0 0 0], radius)), ptClouds);
            %ptClouds = arrayfun(@pctransform, ptClouds, absTforms);
            
            points = vertcat(ptClouds(1:skipFrames:numFrames).Location);
            
            this.Map = pcdownsample( pointCloud(points), 'gridAverage', this.MergeGridStep);
